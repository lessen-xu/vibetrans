...
export interface Scene {
  ...
  desc: string
  hint: string
  /** 书写风格：控制大小写、标点等「打字习惯」，让输出像真人敲出来的 */
  style: string
}
...
export const SCENES: Scene[] = [
  ...
  },
  {
    ...
    style:
      '模仿真实用户的打字习惯：以小写为主（句首、甚至 i 都常小写），句末不加句号，逗号能省则省，宁可松弛也不要工整；不要写成润色过的完整句子',
  },
  {
    id: 'gaming',
    label: '游戏',
    ...
    style: '全小写、极简，句末不加标点，能一个词不用两个词',
  },
  {
    id: 'texting',
    label: '短信私聊',
    desc: '手机短信、WhatsApp、Discord 等一对一消息',
    hint: '像朋友间发消息一样随意简短，可用常见缩写（u、rn、lol、idk 等）',
    style:
      '像真人发消息敲出来的：小写为主，句末不加句号（问号感叹号可以留），标点能省则省，简短随意',
  },
  {
    id: 'email',
    label: '邮件',
...
规则：
- 严格按下面的顺序输出同一句话的 3 种说法，每行一种（每种必须写在同一行内），不要输出档位名称：
  第 1 行（忠实档）：尽量保留原文的信息量、结构和分寸，不增不减，但仍是自然英语
  第 2 行（平衡档）：兼顾原意和地道，可以适度调整说法和详略
  第 3 行（地道档）：优先考虑母语者在这个场景、这个语境下最可能怎么说，可以脱离字面，怎么自然怎么来
- 三档都要像真人在这个场景里真实打出来（或说出来）的话，不是润色过的书面翻译
- 书写风格：${scene.style}
- 不要用破折号（— 或 –），母语者聊天几乎不这么写，改用逗号或拆成短句
